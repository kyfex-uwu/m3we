...
type apiMethod<T extends {[propName:string]:string}> = {
    name:string,
    description?:string|ArrowTemplate,
    props:T,
    returns:string,
    examples:{
        props:T,
...
type apiPage={
    title:string,
    description:string|ArrowTemplate,
    blocks:(apiMethod<any>|string|ArrowTemplate)[],
};
...
export const apiRouter = new Router()
    ...
    .addRoute(":api", (vars)=>html`${(()=>{
        ...
        return html`<div class="api-block">
            ...
            html`<p>${block}</p>` :
            html`<p id="${api.title}_${block.name}"><span class="code">${api.title}.${block.name}(${
                Object.entries(block.props).map(([key, val])=>`${key}:${val}`).join(", ")}):${block.returns}</span>
                    <br>${block.description !== undefined?html`<div>${block.description}</div>`:""}${
                block.examples.map(example=>html`<div class="example">Example: <span class="code">${api.title}.${block.name}(${
                    Object.values(example.props).join(", ")})</span><br>${
                    example.description}</div>`)}</p>`)}</div>`;
    })()}`);

function addApi(page:apiPage){
...
addApi({
    ...
    blocks:[
        ...
        } as apiMethod<{ state: string, propertyName: string }>,
        {
            name:"set",
        ...
        } as apiMethod<{ world:string, pos:string, propertyName: string, value:string }>,
    ]
});
addApi({
    title:"Datastore",
...
addApi({
    ...
    blocks:[
        ...
        } as apiMethod<{ properties: string }>,
        {
            name:"blockPos",
        ...
        } as apiMethod<{ x: string, y:string, z:string }>,
        {
            name:"blockState",
        ...
        } as apiMethod<{ world: string, x:string, y:string, z:string, stack:string, "optional (vx"?:string, vy?:string, vz?:string }>,
    ]
});
addApi({
    title:"Registry",
